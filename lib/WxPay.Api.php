...
class WxPayApi
{
	 ...
	 * @param int $timeOut
	 */
	public static function companypay($inputObj, $timeOut = 10)
	{
		...
		$response = self::postXmlCurl($xml, $url, true, $timeOut);
		//print_r($response);
		$result = WxPayResultsNoSign::Init($response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	 ...
	 * @throws WxPayException
	 */
	private static function postXmlCurl($xml, $url, $useCert = false, $second = 60, $config)
	{
